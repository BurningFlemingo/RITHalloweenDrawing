...
def draw(framebuffer: Framebuffer, viewport: Viewport, vertex_buffer: dict[str, list], vertex_shader: VertexShader, fragment_shader: FragmentShader | None, vertex_count: int, vertex_offset: int):
    ...
    for i in range(0, len(vertices), 3):
        ...
        for triangle in triangles:
            v1_px: Vertex = viewport_transform(viewport, triangle.v1)
            v2_px: Vertex = viewport_transform(viewport, triangle.v2)
            v3_px: Vertex = viewport_transform(viewport, triangle.v3)
            rasterize_triangle(
                framebuffer, fragment_shader, v1_px, v2_px, v3_px)
