    ...
    def load_model(self, model_path: str) -> list[Mesh]:
        ...
        for mesh_asset in mesh_assets:
            ...
            loaded_material: Material = Material(
                ...
                ambient_map=Sampler2D(buffers=[ambient_map], anisotropic_filtering=True).generate_mipmaps(),
                diffuse_map=Sampler2D(buffers=[diffuse_map], anisotropic_filtering=True).generate_mipmaps(),
                specular_map=Sampler2D(buffers=[specular_map], anisotropic_filtering=True).generate_mipmaps(),
